...
export class AddFeedSearchPerformanceIndexes2026042300000 implements MigrationInterface {
  public async up(queryRunner: QueryRunner): Promise<void> {
    if (await this.hasColumns(queryRunner, 'confessions', ['moderation_status', 'created_at', 'is_deleted', 'is_hidden'])) {
    ...
    if (await this.hasColumns(queryRunner, 'confessions', ['gender', 'created_at', 'is_deleted', 'is_hidden'])) {
      await queryRunner.query(`
    ...
    if (await this.hasColumns(queryRunner, 'confessions', ['created_at', 'is_deleted', 'is_hidden', 'moderation_status'])) {
      await queryRunner.query(`
    ...
    if (await this.hasColumns(queryRunner, 'confessions', ['message'])) {
      await queryRunner.query(`
    ...
    if (await this.hasColumns(queryRunner, 'comments', ['parent_id', 'confession_id', 'created_at'])) {
      await queryRunner.query(`
    ...
    if (await this.hasColumns(queryRunner, 'anonymous_users', ['created_at'])) {
      await queryRunner.query(`
